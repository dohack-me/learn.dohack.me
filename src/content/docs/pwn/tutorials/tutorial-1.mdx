...
In C, there are no string classes. Instead, strings are represented as an array of characters. These arrays are also called buffers.
...
Get used to this "scuffed" nature of programs. When dealing with low-level code and programs, it is helpful to think in bytes, not in higher level objects like strings.
...
You managed to change `"changeme"` anyway, since it was "next to" `"hackme"` in the stack. (explanation here)
