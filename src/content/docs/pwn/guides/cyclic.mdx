...
Generate a cyclic pattern with `cyclic`.
- If the program is 32-bit, run `cyclic <length>`
- If the program is 64-bit, run `cyclic -n 8 <length>`
...
- If the program is 32-bit, run `cyclic -l <address>`
- If the program is 64-bit, run `cyclic -n 8 -l <address>`
